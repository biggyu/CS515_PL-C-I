...
fn get_ir(root: &DAGNode, temp_nums: &mut HashMap<DAGNode, usize>, cur_valnum: &mut usize, func_type: Option<String>, llvm_ir: &mut String) -> String {
    match root {
        ...
        DAGNode::Return(var) => {
            ...
            let (var_num, var_chk) = get_tempnum((**var_dag).clone(), temp_nums, cur_valnum);
            llvm_ir.push_str(&format!("\t%t{} = load i64, ptr {}.alloc\n", var_num, var_ir));
            llvm_ir.push_str(&format!("\tret {} %t{}\n", func_type.clone().unwrap_or_else(|| "i64".to_string()), var_num));
            "\n".to_string()
        ...
        DAGNode::TypeDecl{var_type, vars} => {
            ...
            for var in vars {
                let (_, var_dag) = var;
                let var_ir = get_ir(&*var_dag.clone(), temp_nums, cur_valnum, func_type.clone(), llvm_ir);
                llvm_ir.push_str(&format!("\t{}.alloca = alloca {}\n", var_ir, func_type.clone().unwrap_or_else(|| "i64".to_string())));
            }
            "\n".to_string()
        }
        DAGNode::Block(vecs) => {
        ...
        DAGNode::Binop{opr, lhs, rhs} => {
            ...
            let (_, lhs_dag) = lhs;
            let (_, rhs_dag) = rhs;
            let lhs_ir = get_ir(&*lhs_dag.clone(), temp_nums, cur_valnum, func_type.clone(), llvm_ir);
            let rhs_ir = get_ir(&*rhs_dag.clone(), temp_nums, cur_valnum, func_type.clone(), llvm_ir);
